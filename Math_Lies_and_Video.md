...
In his personal blog post `.../xi/posts/2022-09-10-contrast-algorithms/` Tobias is painting himself as a "debunker", and he seems to be escalating his derisive and spurious position in this recent post, which this page is intended to address.
...
Tobias does this and has refuses to acknoledge that the results he claims are "all the same as simple contrast" (paraphrasing) are in fact not at all the same—the implication here is that his true motivation is to confuse and misinform. He has discovered nothing, and asserts falsehoods or his uninformed opinion as if it were fact.

### Debunking Tobias Line by Line
Let's start off with how Tobias does not even know the basics of perception science and he completely misrepresents the formula for Weber contrast.
...
This is laughably wrong. Weber contrast is defined as ` Cw = (Ysymbol - Ybg) / Ybg ` The reality is that Weber (unlike WCAG2) is specific to polarity, as it is _MOST IMPORTANT_ for the adapting field to be the denominator. Apparently Tobias does not know this, and is apparently making these leaps of logic from what appears to be a lack of understanding. 
...
And here the Simple and WCAG are set to Log on the graph. Tobais wants us to believe these curves are "all the same".
...
For actual accurate information on luminance contrast, please see [**NASA's site**](https://colorusage.arc.nasa.gov/luminance_cont.php). 
...
But no matter how you are working to set up a conformance scheme, Tobias' apparent claim that "you can just match thresholds" is completely false. Continuing:
...
His claims that _"everything is the same as simple contrast"_ are incredibly uninformed. As mentioned above, predicting contrast is not something that can be reduced to a single point threshold.
